...
int main() {
    ...
    cout << "Number of bytes: " << dec << inputSize << endl << endl;

    Huffman huff;
    string final = huff.process(input);
    
    cout << "[2] Compression:\n" << endl;

    cout << "As bytes (hex): ";
    for (unsigned char c : final) {
        cout << hex << setw(2) << setfill('0') << (int)(unsigned char)c << " ";
    }

    size_t compressedSize = final.size();
    cout << "\n\nNumber of bytes: " << dec << compressedSize << "\n";

    float reduction = (1.0f - (float)compressedSize / inputSize) * 100;
    cout << fixed << setprecision(2);
    cout << "Text is shorter for " << reduction << "%.\n";
    
    cout << "\n[3] Decompression:\n" << huff.deprocess(final) << endl;
